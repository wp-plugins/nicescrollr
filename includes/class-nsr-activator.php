...
class nsr_activator extends nsr {
	...
	private function check_for_options() {
		...
		$Plugin_Data = new nsr_plugin_data( $this->plugin_name, $this->plugin_domain );

		// Loops trough the option groups and seeds the default options if there is no data in the database yet.
		foreach( $Plugin_Data->get_sections() as $section ) {

			if ( false === $Plugin_Data->get_options( $section ) ) {
				$Plugin_Data->seed_default_options( $section );
			}
		}
	}
}
